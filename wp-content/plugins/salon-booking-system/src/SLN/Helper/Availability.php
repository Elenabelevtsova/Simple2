    ...
    public function getCachedDays() {
        ...
        while ($count > 0) {
            ...
            if ($dayLog->isEnabled()) {
                ...
                $dayLog->addDateLog( $from->toString(), !(isset($day) && $day['status'] == 'full'), __( 'The day out of the booking time range', 'salon-booking-system' ));
            }
        ...
        foreach (SLN_Func::getMinutesIntervals() as $time) {
            ...
            if ($timeLog->isEnabled()) {
                foreach( $avItems->getDateSubset( Salon\Util\Date::create($d) ) as $data){
                    ...
                    if(is_array($data->getData()['from']) && is_array($data->getData()['to'])){
